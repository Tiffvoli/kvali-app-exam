...
export const fetchChatRooms = () => {
   return async (dispatch, getState) => {
      ...
      const response = await fetch(
         `https://kvaliapp-5d4e3-default-rtdb.europe-west1.firebasedatabase.app/chatrooms.json?auth=${token}`,
         {
            method: 'GET',
            headers: {
...
export const newMessage = (chatRoomId, message) => {
   return async (dispatch, getState) => {
      const token = getState().user.token; // accessing token in the state.
      const response = await fetch(
         `https://kvaliapp-5d4e3-default-rtdb.europe-west1.firebasedatabase.app/chatrooms/${chatRoomId}messages.json?auth=${token}`,
         {
            method: 'POST',
